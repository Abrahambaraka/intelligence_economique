...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof data["issue"] === "string") updatable.issue = (data["issue"] as string).trim();
  if (typeof data["pdfUrl"] === "string") updatable.pdfUrl = (data["pdfUrl"] as string).trim();
  ...
  try {
    const updated = await prisma.magazine.update({ where: { id }, data: updatable });
