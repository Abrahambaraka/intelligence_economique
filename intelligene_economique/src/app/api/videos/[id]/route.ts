...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof data["videoUrl"] === "string") updatable.videoUrl = (data["videoUrl"] as string).trim();
  if (typeof data["duration"] === "string") updatable.duration = (data["duration"] as string).trim();
  ...
  try {
    const updated = await prisma.video.update({ where: { id }, data: updatable });
